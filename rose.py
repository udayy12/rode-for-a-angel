...
.angry {
  background: linear-gradient(135deg,#ff9a9e,#ff3d3d);
}
</style>
</head>
<body>
...
function typeText(el, text, speed=40){
  ...
  function t(){
    if(i < text.length){
      el.innerHTML += text[i++];
      typingTimer = setTimeout(t, speed);
    }
  }
  t();
}
...
function chooseYes() {
  ...
  box.classList.add("show","glow");

  typeText(
    document.getElementById("yesMsg"),
    name + ", you deserve all my roses 🥰🌹"
  );

  let roses = "";
  for (let i=0;i<100;i++) roses += "🌹 ";
  document.getElementById("roseWall").innerText = roses;

  for (let i=0;i<70;i++) createPetal();
}
...
function chooseNo() {
  ...
  typeText(
    ...
    name + "... my heart made a tiny dramatic crack sound 💔"
  );
}
...
function restart(){
  ...
  document.getElementById("yesBox").className = "card result";
  document.getElementById("noBox").className = "card result";
  document.getElementById("roseWall").innerText = "";
  document.getElementById("yesMsg").innerText = "";
